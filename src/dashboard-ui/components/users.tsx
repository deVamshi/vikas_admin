...
function App() {
    ...
    for (var i = 0; i < userdata.length; i++) {
        if (userdata[i]["is_buyer"])
            buyers.push(userdata[i])
        else
            sellers.push(userdata[i])
    }
    return (
        <div className="t">
            ...
            <Tabs type="card" className="cardd" >
                <TabPane tab="Seller" key="1" >
                    {sellers.length !== 0 ? <>
                        <Table<Seller> columns={columns} dataSource={sellers} pagination={false} scroll={{ x: 1350 }} />
                        <div className="transactions">
                            <a href="/" > View All Users</a>
                        </div></> : <div style={{ textAlign: "center", fontSize: "25px", marginTop: "120px" }}>Fetching....</div>}
                </TabPane>
                <TabPane tab="Buyer" key="2" >
                    {buyers.length !== 0 ? <>
                        <Table<Buyer> columns={columns1} dataSource={buyers} style={{ width: 1300 }} pagination={false} scroll={{ x: 1350 }} />
                        <div className="transactions">
                            <a href="/" > View All Users</a>
                        </div></> : <div style={{ textAlign: "center", fontSize: "25px", marginTop: "120px" }}>Fetching....</div>}
                </TabPane>
